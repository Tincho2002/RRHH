...
    if df_excel.empty: return pd.DataFrame()
    # La línea original de 'LEGAJO' a numérico se omite, ya que la trataremos como string más abajo
...
        df_excel['CECO'] = df_excel['CECO'].astype(int).astype(str)
    ...
    # --- INICIO DE MODIFICACIÓN 1 (Añadir Bloque LEGAJO) ---
...
        df_excel['LEGAJO'] = df_excel['LEGAJO'].apply(lambda x: str(int(x)) if pd.notna(x) and isinstance(x, (int, float, np.number)) else ('no disponible' if pd.isna(x) else str(x)))
    ...
    # --- INICIO DE MODIFICACIÓN 2 (Añadir LEGAJO a la lista de columnas) ---
...
        if col not in df_excel.columns: df_excel[col] = 'no disponible'
        ...
        # --- INICIO DE MODIFICACIÓN 3 (Omitir reprocesamiento de LEGAJO/CECO) ---
...
    
    # --- INICIO DE MODIFICACIÓN 4 (Añadir LEGAJO al diccionario de filtros) ---
    filter_cols_config = {
...
I               df_mapa_agg.dropna(subset=['Latitud', 'Longitud'], inplace=True)
...
                else:
                    comp_col1, comp_col2 = st.columns([3, 2])content 
                    with comp_col1:
...
section                                st.info("Intente recargar la página o seleccionar un período con menos datos.")
                        # 2. Cerramos el div
...
A                             if 'FC' not in pivot_table.columns: pivot_table['FC'] = 0
...
Note                            df_final_table = pd.concat([pivot_table.reset_index(), total_row], ignore_index=True)
...
section   if tab_map_individual and period_to_display:
...
                    df_mapa_agg = df_mapa_data.groupby(['Distrito', 'Latitud', 'Longitud']).size().reset_index(name='Dotacion_Total')
an                     if df_mapa_agg.empty: st.warning("No hay datos de ubicación para mostrar en el mapa para la selección actual.")
                    else:
                    s     mapbox_access_token = "pk.eyJ1Ijoic2FuZHJhcXVldmVkbyIsImEiOiJjbWYzOGNkZ2QwYWg0MnFvbDJucWc5d3VwIn0.bz6E-qxAwk6ZFPYohBsdMw"
                        px.set_mapbox_access_token(mapbox_access_token)
...
            with col_table:
                pivot_table = pd.pivot_table(data=df_mapa_display, index='Distrito', columns='Relación', aggfunc='size', fill_value=0)
A                 if 'Convenio' not in pivot_table.columns: pivot_table['Convenio'] = 0
...
e               st.altair_chart(bars_edad + total_labels_edad, use_container_width=True)
            with col_table_edad:
...
G            	generate_download_buttons(antiguedad_table.reset_index(), f'distribucion_antiguedad_{periodo_a_mostrar_edad}', key_suffix="_antiguedad")
...
Click        	df_periodo_desglose = filtered_df[filtered_df['Periodo'] == periodo_a_mostrar_desglose]
...
G            	st.dataframe(table_data.style.format({"Cantidad": format_integer_es}))
...
s     	    display_df['LEGAJO'] = display_df['LEGAJO'].apply(lambda x: format_integer_es(int(x)) if x.isdigit() else x)
