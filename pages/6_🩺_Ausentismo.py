...
if uploaded_file is not None:
    ...
    if 'au_selections_v14' not in st.session_state or st.sidebar.button("🔄 Resetear Filtros", use_container_width=True):
        st.session_state.au_selections_v14 = {k: list(v) for k, v in all_possible_options.items()}
    ...
    for col, label in filter_dict.items():
        ...
        current_defaults = [x for x in st.session_state.au_selections_v14.get(col, opts) if x in opts]
        ...
        st.session_state.au_selections_v14[col] = sel
    ...
    sel_periodos = [p for p in periodos_ordenados if p in st.session_state.au_selections_v14.get('Periodo_Label', [])]
    ...
    # Opciones de estilos de mapas de Mapbox
    map_styles = {
    ...
    tab_iau, tab_geo_d, tab_geo_h, tab_dias_caidos, tab_horas_caidas, tab_matriz_hc, tab_cronologia, tab_gantt, tab_dimensiones, tab1, tab2, tab4 = st.tabs([
        ...
        "👥 Análisis Dimensional (Relación, Sexo, Áreas)",
    ...
    with tab_geo_d:
        ...
        if not df_geo_distritos.empty:
            ...
            with col_geo_right:
                # Selector de estilo de mapa idéntico a Dotación
                sel_estilo_d = st.selectbox(
                    "Selecciona el estilo del mapa:",
                    options=list(map_styles.keys()),
                    index=0,
                    key="sel_map_style_d"
                )
                chosen_style_d = map_styles[sel_estilo_d]
    ...
    with tab_geo_h:
        ...
        if not df_geo_distritos.empty:
            ...
            with col_geo_right_h:
                # Selector de estilo de mapa idéntico a Dotación
                sel_estilo_h = st.selectbox(
                    "Selecciona el estilo del mapa:",
                    options=list(map_styles.keys()),
                    index=0,
                    key="sel_map_style_h"
                )
                chosen_style_h = map_styles[sel_estilo_h]
    ...
    with tab_cronologia:
        ...
        # En Looker Studio, la tabla de licencias acumula hasta Julio (7 períodos)
        # Excluimos agosto solo de las licencias de días continuos largos para mantener Figueroa en 1.785
        if 'Periodo_Label' in df_cron_source.columns:
            mask_lic_larga = (
                (df_cron_source['Tipo'] == 'Licencia') & 
                (df_cron_source['Total (D)'] > 100) & 
                (df_cron_source['Periodo_Label'].astype(str).str.lower().str.startswith('ago'))
            )
            df_cron_source = df_cron_source[~mask_lic_larga].copy()

        # Normalizar textos y nulos antes de agrupar
        ...
        if not df_cron_agg.empty:
            ...
            # --- FILA DE TOTAL GENERAL (IDÉNTICA A LOOKER STUDIO) ---
            ...
            df_cron_display = pd.concat([df_cron_display, fila_total], ignore_index=True)
    ...
    # =========================================================================
    # --- TAB ANÁLISIS DIMENSIONAL: RELACIÓN, SEXO, ÁREAS (ESTRUCTURA LOOKER) ---
    # =========================================================================
    with tab_dimensiones:
        st.subheader("Análisis Dimensional de Ausentismo y Dotación")
        ...
        dim_opciones = {
            ...
            "Distrito": "Distrito",
            "Nivel": "Nivel",
            "Clasificación Ministerial": "Ministerio"
        }
        
        sel_dim_label = st.selectbox(
            "Seleccionar Dimensión de Análisis:",
            options=list(dim_opciones.keys()),
            index=0,
            key="sb_dim_analisis"
        )
        dim_col = dim_opciones[sel_dim_label]

        col_au_ok = dim_col in filtered_df.columns
        col_dot_ok = (filtered_dot is not None) and (dim_col in filtered_dot.columns)

        if not col_au_ok:
            st.warning(f"La columna '{dim_col}' no está presente en los datos de ausentismo.")
        else:
            categorias = sorted([c for c in filtered_df[dim_col].unique() if str(c) not in ['no disponible', 'nan', 'None']])

            # --- FILA 1: CASOS POR MES Y DOTACIÓN POR MES ---
            col_r1_1, col_r1_2 = st.columns(2)

            with col_r1_1:
                st.markdown(f"##### Cantidad de Casos Por Mes y Por Tipo de Licencia y por {sel_dim_label}")
                df_casos_dim = pd.pivot_table(
                    filtered_df,
                    index=['Tipo', dim_col],
                ...
                cols_p_presentes = [p for p in sel_periodos if p in df_casos_dim.columns]
                df_casos_dim = df_casos_dim.reindex(columns=cols_p_presentes, fill_value=0)
                
                df_casos_dim['Total'] = filtered_df.groupby(['Tipo', dim_col])['Legajo'].nunique()
                
                tot_casos_cols = {p: filtered_df[filtered_df['Periodo_Label'] == p]['Legajo'].nunique() for p in cols_p_presentes}
                tot_casos_cols['Total'] = filtered_df['Legajo'].nunique()
                df_tot_casos = pd.DataFrame(tot_casos_cols, index=pd.MultiIndex.from_tuples([('Total', '')], names=['Tipo', dim_col]))
                
                df_casos_final = pd.concat([df_casos_dim, df_tot_casos])
                st.dataframe(df_casos_final.style.format(format_integer_es), use_container_width=True)

            with col_r1_2:
                st.markdown(f"##### Dotación por Mes y por {sel_dim_label}")
                if col_dot_ok:
                    df_dot_dim = pd.pivot_table(
                        filtered_dot,
                        index=dim_col,
                        columns='Periodo_Label',
                        values='Legajo',
                    ...
                    cols_dot_presentes = [p for p in sel_periodos if p in df_dot_dim.columns]
                    df_dot_dim = df_dot_dim.reindex(columns=cols_dot_presentes, fill_value=0)
                    
                    tot_dot_cols = {p: filtered_dot[filtered_dot['Periodo_Label'] == p]['Legajo'].nunique() for p in cols_dot_presentes}
                    df_tot_dot = pd.DataFrame(tot_dot_cols, index=['Total'])
                    df_dot_final = pd.concat([df_dot_dim, df_tot_dot])
                    st.dataframe(df_dot_final.style.format(format_integer_es), use_container_width=True)
                else:
                    st.info("Dotación no disponible para esta variable.")

            st.markdown("---")

            # --- FILA 2: AUSENTISMO DÍAS Y HORAS POR MES ---
            cat_list = [c for c in categorias if c != 'no disponible']
            matriz_dias_data = []
            matriz_horas_data = []

            for cat in cat_list:
                row_d = {sel_dim_label: cat}
                row_h = {sel_dim_label: cat}
                cap_d_acum = 0
                cap_h_acum = 0
                dias_acum = 0
                horas_acum = 0
                ...
                for p in sel_periodos:
                    dh = dias_habiles_por_mes.get(p, 21)
                    sub_au_cp = filtered_df[(filtered_df[dim_col] == cat) & (filtered_df['Periodo_Label'] == p)]
                    d_val = sub_au_cp['Total (D)'].sum()
                    h_val = pd.to_numeric(sub_au_cp['Total (H)'], errors='coerce').fillna(0.0).sum()

                    if col_dot_ok:
                        dot_val = filtered_dot[(filtered_dot[dim_col] == cat) & (filtered_dot['Periodo_Label'] == p)]['Legajo'].nunique()
                    else:
                        dot_val = sub_au_cp['Legajo'].nunique()

                    cap_d_p = dot_val * dh
                    cap_h_p = cap_d_p * 8

                    row_d[p] = (d_val / cap_d_p * 100) if cap_d_p > 0 else 0.0
                    row_h[p] = (h_val / cap_h_p * 100) if cap_h_p > 0 else 0.0
                ...
                row_d['Total'] = (dias_acum / cap_d_acum * 100) if cap_d_acum > 0 else 0.0
                row_h['Total'] = (horas_acum / cap_h_acum * 100) if cap_h_acum > 0 else 0.0

                matriz_dias_data.append(row_d)
                matriz_horas_data.append(row_h)

            df_matriz_d = pd.DataFrame(matriz_dias_data).set_index(sel_dim_label)
            df_matriz_h = pd.DataFrame(matriz_horas_data).set_index(sel_dim_label)

            tot_dias_row = {}
            tot_horas_row = {}
            for p in sel_periodos:
                val_d = df_evo[df_evo['Periodo'] == p]['Índice_Días'].values
                val_h = df_evo[df_evo['Periodo'] == p]['Índice_Horas'].values
                tot_dias_row[p] = val_d[0] if len(val_d) > 0 else 0.0
                tot_horas_row[p] = val_h[0] if len(val_h) > 0 else 0.0
            tot_dias_row['Total'] = tasa_dias
            tot_horas_row['Total'] = tasa_horas

            df_matriz_d_display = pd.concat([df_matriz_d, pd.DataFrame(tot_dias_row, index=['Total'])])
            df_matriz_h_display = pd.concat([df_matriz_h, pd.DataFrame(tot_horas_row, index=['Total'])])

            col_r2_1, col_r2_2 = st.columns(2)

            with col_r2_1:
                st.markdown(f"##### Ausentismo en Días (Licencias) por {sel_dim_label} y por Mes")
                fmt_pct_cols = {col: lambda x: format_percentage_es(x, 2) for col in sel_periodos + ['Total']}
                st.dataframe(df_matriz_d_display.style.format(fmt_pct_cols), use_container_width=True)

            with col_r2_2:
                st.markdown(f"##### Ausentismo en Horas (Novedades) por {sel_dim_label} y por Mes")
                st.dataframe(df_matriz_h_display.style.format(fmt_pct_cols), use_container_width=True)

            st.markdown("---")

            # --- FILA 3: GRÁFICOS DE BARRAS AGRUPADAS POR MES Y POR CATEGORÍA ---
            paleta_dim = ['#2563eb', '#dc2626', '#10b981', '#f59e0b', '#8b5cf6', '#ec4899', '#06b6d4', '#84cc16']

            col_r3_1, col_r3_2 = st.columns(2)

            with col_r3_1:
                st.markdown(f"##### Índice Ausentismo (Días) por Período y {sel_dim_label}")
                fig_bar_d_dim = go.Figure()
                max_d_y = 0.0

                for idx, cat in enumerate(cat_list):
                    vals_d = [df_matriz_d.loc[cat, p] if (cat in df_matriz_d.index and p in df_matriz_d.columns) else 0.0 for p in sel_periodos]
                    if vals_d:
                        max_d_y = max(max_d_y, max(vals_d))
                    fig_bar_d_dim.add_trace(go.Bar(
                        x=sel_periodos,
                        y=vals_d,
                        name=str(cat),
                        text=[f"{v:.2f}%".replace('.', ',') if v > 0 else "" for v in vals_d],
                        textposition='outside',
                        textfont=dict(size=9),
                        marker_color=paleta_dim[idx % len(paleta_dim)]
                    ))

                fig_bar_d_dim.update_layout(
                    barmode='group',
                    yaxis=dict(title="Índice Ausentismo (Días)", range=[0, max_d_y * 1.35 if max_d_y > 0 else 10]),
                    xaxis=dict(categoryorder='array', categoryarray=sel_periodos),
                    legend=dict(orientation="h", y=1.15, x=0.5, xanchor='center'),
                    height=380,
                    margin=dict(t=30, b=40, l=10, r=10)
                )
                st.plotly_chart(fig_bar_d_dim, use_container_width=True)

            with col_r3_2:
                st.markdown(f"##### Índice Ausentismo (Horas) por Período y {sel_dim_label}")
                fig_bar_h_dim = go.Figure()
                max_h_y = 0.0

                for idx, cat in enumerate(cat_list):
                    vals_h = [df_matriz_h.loc[cat, p] if (cat in df_matriz_h.index and p in df_matriz_h.columns) else 0.0 for p in sel_periodos]
                    if vals_h:
                        max_h_y = max(max_h_y, max(vals_h))
                    fig_bar_h_dim.add_trace(go.Bar(
                        x=sel_periodos,
                        y=vals_h,
                        name=str(cat),
                        text=[f"{v:.2f}%".replace('.', ',') if v > 0 else "" for v in vals_h],
                        textposition='outside',
                        textfont=dict(size=9),
                        marker_color=paleta_dim[idx % len(paleta_dim)]
                    ))

                fig_bar_h_dim.update_layout(
                    barmode='group',
                    yaxis=dict(title="Índice Ausentismo (Horas)", range=[0, max_h_y * 1.35 if max_h_y > 0 else 5]),
                    xaxis=dict(categoryorder='array', categoryarray=sel_periodos),
                    legend=dict(orientation="h", y=1.15, x=0.5, xanchor='center'),
                    height=380,
                    margin=dict(t=30, b=40, l=10, r=10)
                )
                st.plotly_chart(fig_bar_h_dim, use_container_width=True)
