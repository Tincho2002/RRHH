...
if uploaded_file is not None:
    ...
    # Inicialización de filtros con todos los meses disponibles incluidos
    if 'au_selections_v11' not in st.session_state or st.sidebar.button("🔄 Resetear Filtros", use_container_width=True):
        st.session_state.au_selections_v11 = {k: list(v) for k, v in all_possible_options.items()}
    ...
    for col, label in filter_dict.items():
        ...
        current_defaults = [x for x in st.session_state.au_selections_v11.get(col, opts) if x in opts]
        ...
        st.session_state.au_selections_v11[col] = sel
    ...
    sel_periodos = [p for p in periodos_ordenados if p in st.session_state.au_selections_v11.get('Periodo_Label', [])]
