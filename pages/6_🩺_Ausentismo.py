...
if uploaded_file is not None:
    ...
    with tab_cronologia:
            ...
            # Filtramos para las licencias en días los meses posteriores a Julio si estuviesen seleccionados globalmente
            if 'Periodo_Label' in df_cron_source.columns:
                # Si hay meses cerrados en licencias, conservamos el acumulado de Looker
                meses_lic_validos = [p for p in df_cron_source['Periodo_Label'].unique() if not str(p).lower().startswith('ago')]
                if meses_lic_validos:
                    # Aplicamos la exclusión de agosto solo a las licencias de días continuos largos
                    mask_lic_larga = (df_cron_source['Tipo'] == 'Licencia') & (df_cron_source['Total (D)'] > 100) & (df_cron_source['Periodo_Label'].str.lower().startswith('ago'))
