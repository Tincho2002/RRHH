...
if uploaded_file is not None:
    ...
    # Inicialización de estado con todos los meses disponibles
    if 'au_selections_v12' not in st.session_state or st.sidebar.button("🔄 Resetear Filtros", use_container_width=True):
        st.session_state.au_selections_v12 = {k: list(v) for k, v in all_possible_options.items()}
    ...
    for col, label in filter_dict.items():
        ...
        current_defaults = [x for x in st.session_state.au_selections_v12.get(col, opts) if x in opts]
        ...
        st.session_state.au_selections_v12[col] = sel
    ...
    sel_periodos = [p for p in periodos_ordenados if p in st.session_state.au_selections_v12.get('Periodo_Label', [])]
    ...
    with tab_dimensiones:
        ...
        # Validamos que la columna exista en ambos DataFrames
        ...
        if not col_au_ok:
            ...
            with col_r1_1:
                ...
                # Pivot de Casos (Agentes únicos o Casos)
                ...
                # Reindexar columnas por sel_periodos
                ...
                # Fila de Total general por columna
            ...
            # Precalculamos índices matriciales
            ...
            for cat in cat_list:
                for p in sel_periodos:
                    val_d = df_matriz_d.loc[cat, p] if (cat in df_matriz_d.index and p in df_matriz_d.columns) else 0.0
                    val_h = df_matriz_h.loc[cat, p] if (cat in df_matriz_h.index and p in df_matriz_h.columns) else 0.0
                    chart_rows_d.append({'Periodo': p, sel_dim_label: cat, 'Índice (Días)': val_d})
                    chart_rows_h.append({'Periodo': p, sel_dim_label: cat, 'Índice (Horas)': val_h})

            df_chart_d = pd.DataFrame(chart_rows_d)
            df_chart_h = pd.DataFrame(chart_rows_h)

            col_r3_1, col_r3_2 = st.columns(2)

            with col_r3_1:
                st.markdown(f"##### Índice Ausentismo (Días) por Período y {sel_dim_label}")
                if not df_chart_d.empty:
                    fig_bar_d_dim = px.bar(
                        df_chart_d,
                        x='Periodo',
                        y='Índice (Días)',
                        color=sel_dim_label,
                        barmode='group',
                        text='Índice (Días)',
                        color_discrete_sequence=['#2563eb', '#dc2626', '#10b981', '#f59e0b', '#8b5cf6', '#ec4899']
                    )
                    fig_bar_d_dim.update_traces(
                        texttemplate='%{text:.2f}%',
                        textposition='outside',
                        textfont=dict(size=9)
                    )
                    max_d_y = df_chart_d['Índice (Días)'].max()
                    fig_bar_d_dim.update_layout(
                        yaxis=dict(title="Índice Ausentismo (Días)", range=[0, max_d_y * 1.3 if max_d_y > 0 else 10]),
            ...
            with col_r3_2:
                ...
                if not df_chart_h.empty:
                    fig_bar_h_dim = px.bar(
                        df_chart_h,
                        x='Periodo',
                        y='Índice (Horas)',
                        color=sel_dim_label,
                    ...
                    fig_bar_h_dim.update_traces(
                        texttemplate='%{text:.2f}%',
                        textposition='outside',
                        textfont=dict(size=9)
                    )
                    max_h_y = df_chart_h['Índice (Horas)'].max()
                    fig_bar_h_dim.update_layout(
                        yaxis=dict(title="Índice Ausentismo (Horas)", range=[0, max_h_y * 1.3 if max_h_y > 0 else 5]),
                        xaxis=dict(categoryorder='array', categoryarray=sel_periodos),
                        legend=dict(orientation="h", y=1.15, x=0.5, xanchor='center'),
                        height=380,
                        margin=dict(t=30, b=40, l=10, r=10)
