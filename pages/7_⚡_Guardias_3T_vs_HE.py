...
if uploaded_file is not None:
    ...
    if 'g3t_selections_v3' not in st.session_state or st.sidebar.button("🔄 Resetear Filtros", use_container_width=True):
        st.session_state.g3t_selections_v3 = {k: list(v[1]) for k, v in filter_dict.items()}
    ...
    for col, (label, opts) in filter_dict.items():
        curr_defaults = [x for x in st.session_state.g3t_selections_v3.get(col, opts) if x in opts]
        sel = st.sidebar.multiselect(label, options=opts, default=curr_defaults, key=f"sel_g3t_v3_{col}")
        st.session_state.g3t_selections_v3[col] = sel
    ...
    with tab_geo:
        ...
        sel_estilo_g3t = st.selectbox(
            "Selecciona el estilo del mapa:",
            options=list(map_styles.keys()),
            index=0,
            key="sel_map_style_g3t"
        )
        chosen_style_g3t = map_styles[sel_estilo_g3t]
